...
const MpesaIcon = ({ size = "md" }) => {
  ...
  return (
    <div className={`${sizes[size]} bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center shadow-md shadow-green-200 flex-shrink-0`}>
      <span className="text-white font-bold text-[8px] sm:text-[10px] tracking-tight">M-PESA</span>
    </div>
  );
};
...
const paymentMethods = [
  ...
  },
  {
    ...
    icon: (size) => {
      ...
      return (
        <div className={`${sizes[size]} bg-gradient-to-br from-blue-600 to-blue-700 rounded-lg flex items-center justify-center shadow-md shadow-blue-200 flex-shrink-0`}>
          <CreditCard className={`${size === 'sm' ? 'w-3 h-3' : size === 'md' ? 'w-4 h-4' : 'w-5 h-5'} text-white`} />
        </div>
      );
    },
    color: "blue",
  ...
  },
  {
    ...
    icon: (size) => {
      const sizes = { sm: "w-6 h-6", md: "w-8 h-8", lg: "w-10 h-10" };
      return (
        <div className={`${sizes[size]} bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-lg flex items-center justify-center shadow-md shadow-indigo-200 flex-shrink-0`}>
          <FaPaypal className={`${size === 'sm' ? 'text-xs' : size === 'md' ? 'text-sm' : 'text-lg'} text-white`} />
        </div>
      );
    },
    color: "indigo",
  ...
  },
  {
    ...
    icon: (size) => {
      const sizes = { sm: "w-6 h-6", md: "w-8 h-8", lg: "w-10 h-10" };
      return (
        <div className={`${sizes[size]} bg-gradient-to-br from-emerald-600 to-emerald-700 rounded-lg flex items-center justify-center shadow-md shadow-emerald-200 flex-shrink-0`}>
          <Banknote className={`${size === 'sm' ? 'w-3 h-3' : size === 'md' ? 'w-4 h-4' : 'w-5 h-5'} text-white`} />
        </div>
      );
    },
    color: "emerald",
...
const InputField = ({ 
  ...
  <div className={`relative group ${half ? '' : 'sm:col-span-2'}`}>
    ...
    <input
      ...
      required={required}
      className={`w-full bg-gray-50 border-2 ${error ? 'border-red-300 focus:border-red-500 focus:ring-red-100' : 'border-gray-200 focus:border-indigo-600 focus:ring-indigo-100'} rounded-xl text-gray-900 placeholder:text-gray-400 focus:bg-white focus:ring-4 outline-none transition-all duration-300 font-medium ${isMobile ? 'pl-9 pr-3 py-2.5 text-xs' : 'pl-11 pr-4 py-3.5 text-sm'}`}
    />
    {required && !value && (
      <span className="absolute right-2.5 sm:right-3 top-1/2 -translate-y-1/2 text-xs text-red-400 font-medium">
        *
      </span>
    )}
    {error && (
...
const ProgressStep = ({ number, title, active, completed, isMobile }) => (
  <div className="flex items-center gap-1.5 sm:gap-2">
    <div className={`${isMobile ? 'w-6 h-6 text-[10px]' : 'w-8 h-8 text-xs'} rounded-full flex items-center justify-center font-bold transition-all duration-300 ${
      ...
      active ? "bg-indigo-600 text-white ring-2 sm:ring-4 ring-indigo-100 shadow-md shadow-indigo-200" : 
...
const SectionHeader = ({ icon: Icon, title, subtitle, gradient, isMobile }) => (
  <div className={`bg-gradient-to-r ${gradient} ${isMobile ? 'px-3 py-2.5' : 'px-4 py-3 sm:px-6 sm:py-4'}`}>
    <div className="flex items-center gap-2 sm:gap-3">
      <div className={`${isMobile ? 'w-7 h-7' : 'w-9 h-9'} bg-white/20 rounded-lg flex items-center justify-center flex-shrink-0`}>
        <Icon className="text-white text-xs sm:text-base" />
      </div>
      <div className="min-w-0">
        <h2 className="text-white font-bold text-xs sm:text-base truncate">{title}</h2>
        <p className={`${isMobile ? 'text-[9px]' : 'text-xs sm:text-sm'} text-white/80 truncate hidden sm:block`}>{subtitle}</p>
      </div>
    </div>
  </div>
);
...
const CheckoutPage = () => {
  ...
  const validateForm = () => {
    ...
    // Fixed email regex - removed double escaping
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 pb-20 sm:pb-0">
      ...
      <div className="bg-white shadow-sm border-b sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 py-2 sm:py-4">
          <div className="flex items-center justify-between">
            <Link to="/cart" className="flex items-center gap-1 sm:gap-2 text-gray-600 hover:text-gray-900 transition-colors p-1 -ml-1 sm:ml-0">
              <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5" />
              <span className="font-medium text-xs sm:text-sm hidden sm:inline">Back to Cart</span>
            ...
            <div className="flex items-center gap-1 sm:gap-2 text-green-600">
              <FaShieldAlt size={isMobile ? 12 : 16} />
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 py-3 sm:py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 sm:gap-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-2 gap-2 sm:gap-4">
                  {paymentMethods.map((method) => (
                    <div
                      ...
                      className={`relative cursor-pointer rounded-lg sm:rounded-xl border-2 p-2 sm:p-4 transition-all duration-300 ${
                    ...
                    >
                      {method.popular && !isMobile && (
                        <span className="absolute -top-2 left-2 sm:left-3 bg-gradient-to-r from-orange-500 to-red-500 text-white text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full shadow-sm">
                          POPULAR
                        </span>
                      )}
                      ...
                      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-2 sm:gap-3">
                        ...
                        <div className="flex-1 min-w-0 text-center sm:text-left">
                          <div className="flex items-center justify-center sm:justify-between gap-1 sm:mb-0.5">
                            ...
                            {paymentMethod === method.id && (
                              <FaCheckCircle className={`${method.textColor} flex-shrink-0 text-xs sm:text-lg`} />
                            )}
                          </div>
          ...
          <div className="lg:col-span-5">
            ...
            <div className="lg:hidden mb-2">
              ...
              >
                <div className="flex items-center gap-1.5 sm:gap-2">
                  ...
                  <span className="font-semibold text-gray-900 text-xs sm:text-sm">Order Summary</span>
                  <span className="bg-gray-100 text-gray-600 text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 rounded-full">
                    {cartItems.length}
                  </span>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <span className="font-bold text-gray-900 text-xs sm:text-sm">{displayKESCurrency(totalAmount)}</span>
                  <ChevronDown className={`w-4 h-4 sm:w-5 sm:h-5 text-gray-400 transition-transform ${showOrderSummary ? 'rotate-180' : ''}`} />
                </div>
              </button>
            </div>
              ...
              <div className={`${isMobile ? 'p-3' : 'p-4'}`}>
                ...
                <div className="space-y-2 sm:space-y-3 mb-3 sm:mb-4 max-h-40 sm:max-h-52 overflow-y-auto">
                  {cartItems.map((item, index) => (
                    <div key={index} className="flex gap-2 sm:gap-3 p-2 bg-gray-50 rounded-lg sm:rounded-xl">
                      <div className="relative flex-shrink-0">
                        <img
                          src={item?.productId?.productImage?.[0] || "/fallback-image.png"}
                          alt={item?.productId?.productName}
                          className="w-10 h-10 sm:w-14 sm:h-14 rounded-lg object-cover bg-white"
                        />
                ...
                {/* Calculations */}
                <div className="space-y-1.5 sm:space-y-2 py-2 sm:py-3 border-t border-gray-100">
                  ...
                  <div className="flex justify-between text-[11px] sm:text-xs">
                    <span className="text-gray-600 flex items-center gap-1 sm:gap-1.5">
                      <FaTruck className="text-blue-600 text-[10px] sm:text-xs" />
                      Shipping
                    </span>
                ...
                <button
                  ...
                  className="hidden sm:flex w-full mt-4 sm:mt-6 bg-gradient-to-r from-gray-900 to-gray-800 text-white py-3 sm:py-4 rounded-xl sm:rounded-2xl font-bold text-sm sm:text-base hover:from-gray-800 hover:to-gray-700 disabled:opacity-70 disabled:cursor-not-allowed transition-all duration-300 shadow-lg hover:shadow-xl items-center justify-center gap-2 sm:gap-3 group"
                >
                  {placingOrder ? (
                    <>
                ...
                <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-100">
                  <div className="flex items-center justify-center gap-2 sm:gap-3 text-[9px] sm:text-[10px] text-gray-500">
                    <div className="flex items-center gap-0.5 sm:gap-1">
                      <FaLock className="text-green-600" size={isMobile ? 9 : 11} />
                      <span>SSL</span>
                    </div>
                    <div className="flex items-center gap-0.5 sm:gap-1">
                      <FaShieldAlt className="text-blue-600" size={isMobile ? 9 : 11} />
                      <span>Protected</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="hidden sm:grid mt-3 sm:mt-4 grid-cols-3 gap-2 sm:gap-3">
              <div className="bg-white rounded-xl p-2.5 sm:p-4 text-center border border-gray-100 shadow-sm">
                <FaCcVisa className="w-5 h-5 sm:w-7 sm:h-7 mx-auto text-blue-600 mb-1 sm:mb-2" />
                <span className="text-[10px] sm:text-xs text-gray-600">Visa</span>
              </div>
              <div className="bg-white rounded-xl p-2.5 sm:p-4 text-center border border-gray-100 shadow-sm">
                <FaCcMastercard className="w-5 h-5 sm:w-7 sm:h-7 mx-auto text-red-600 mb-1 sm:mb-2" />
                <span className="text-[10px] sm:text-xs text-gray-600">Mastercard</span>
              </div>
              <div className="bg-white rounded-xl p-2.5 sm:p-4 text-center border border-gray-100 shadow-sm">
                <FaShieldAlt className="w-5 h-5 sm:w-7 sm:h-7 mx-auto text-green-600 mb-1 sm:mb-2" />
                <span className="text-[10px] sm:text-xs text-gray-600">Secure</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-2 shadow-lg z-50 safe-area-bottom">
        <div className="flex items-center justify-between gap-2 mb-1.5">
          <div>
            <p className="text-[9px] text-gray-500">Total to pay</p>
            <p className="text-base sm:text-lg font-bold text-gray-900">{displayKESCurrency(totalAmount)}</p>
          </div>
          <button
            onClick={handlePlaceOrder}
            disabled={placingOrder}
            className={`flex-1 max-w-[160px] sm:max-w-[200px] py-2 sm:py-3 rounded-lg sm:rounded-xl font-bold text-white shadow-md sm:shadow-lg transition-all active:scale-[0.98] flex items-center justify-center gap-1.5 sm:gap-2 text-xs sm:text-sm ${
              placingOrder
